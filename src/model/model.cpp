...
LightShaderData light_data[NUM_LIGHTS] = {
    { glf3d::POINT, glm::vec3(1.2f, 0.5f, 0.0f), glm::vec3(1.0f,  1.0f, -1.0f),
      glm::vec3(1, 0, 0), 0, 0, 1.0f, 1.0f, 0.05f },
    { glf3d::POINT, glm::vec3(1.6f, 0.5f, 0.0f), glm::vec3(1.0f,  1.0f, -1.0f),
      glm::vec3(0, 1, 0), 0, 0, 1.0f, 1.0f, 0.05f },
    { glf3d::POINT, glm::vec3(2.0f, 0.5f, 0.0f), glm::vec3(1.0f,  1.0f, -1.0f),
      glm::vec3(0, 0, 1), 0, 0, 1.0f, 1.0f, 0.05f },
    { glf3d::POINT, glm::vec3(2.4f, 0.5f, 0.0f), glm::vec3(1.0f,  1.0f, -1.0f),
      glm::vec3(1, 1, 0), 0, 0, 1.0f, 1.0f, 0.05f },
    { glf3d::POINT, glm::vec3(0.0f, 2.0f, 0.0f), glm::vec3(1.0f,  1.0f, -1.0f),
      glm::vec3(1.0f), 0, 0, 1.0f, 1.0f, 0.05f }
};
...
class Model: public glf::BaseApp {
 public:
    Model() {
    ...
    void startup() {
        // Initiate shaders
        light_shader.load("media/model/shaders/light.vert",
            glf::Shader::VERTEX);
        light_shader.load("media/model/shaders/light.frag",
            glf::Shader::FRAGMENT);
        light_shader.compile();

        scene_shader.load("media/model/shaders/scene.vert",
            glf::Shader::VERTEX);
        scene_shader.load("media/model/shaders/scene.frag",
            glf::Shader::FRAGMENT);
        scene_shader.compile(); scene_shader.use();

        text_shader.load("media/model/shaders/text.vert",
            glf::Shader::VERTEX);
        text_shader.load("media/model/shaders/text.frag",
            glf::Shader::FRAGMENT);
        text_shader.compile();
...
#undef uni_loc
        ...
        // Setup objects
        crysis.load("media/data/models/cube/cube.obj", true);
        crysis.material.shininess(100.0f);
        crysis.translate(glm::vec3(0.0f, 0.501f, 0.0f));
        crysis.scalef(0.5f);
        plane.load("media/data/models/vents/vent.obj", true);
        plane.material.shininess(100.0f);
        plane.scalef(2.5f);
        ...
        // Animation
        for (GLint i = 0; i < NUM_LIGHTS - 1; ++i) {
            lightAnim[i].set(0.0, 360.0, i * 2.0f + 1.0f);
...
#define _vp(f) glm::value_ptr(f)
    ...
    void render(double elapsedTime, double duration) {
        ...
        glm::mat3 normal_mat =
            glm::transpose(glm::inverse(glm::mat3(crysis.modelMat())));
        ...
        glUniformMatrix3fv(loc_scene_normal, 1, GL_FALSE, _vp(normal_mat));
        ...
        trans_mat = proj_view_mat * plane.modelMat();
        normal_mat =
            -glm::transpose(glm::inverse(glm::mat3(plane.modelMat())));
        glUniformMatrix4fv(loc_scene_model, 1, GL_FALSE, _vp(plane.modelMat()));
        glUniformMatrix4fv(loc_scene_trans, 1, GL_FALSE, _vp(trans_mat));
        glUniformMatrix3fv(loc_scene_normal, 1, GL_FALSE, _vp(normal_mat));
        glUniform1f(loc_scene_amb, plane.material.ambience());
        glUniform1f(loc_scene_shine, plane.material.shininess());
        glUniform1f(loc_scene_specint, plane.material.specularIntensity());
        plane.render(&shader_tex_info);
...
#undef _vp
    ...
    void update_state(double duration) {
        ...
        for (GLint i = 0; i < NUM_LIGHTS - 1; ++i) {
            if (light[i].type() != glf3d::POINT) continue;
            light[i].rotateAnchor(
                glm::radians(lightAnim[i].value()), kModelRotAxis, kWorldOrigin);
        }
 ...
 private:
    ...
    enum ViewState { VIEW_OBJECT, VIEW_WIREFRAME };
    ...
    anim::PropertyAnimator<GLfloat> lightAnim[NUM_LIGHTS-1];
};
