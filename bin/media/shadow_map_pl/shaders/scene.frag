...
const vec3 kPCFSamples[20] = vec3[](
...
// PCF (20x Samples)
float calc_shadow(vec3 lpos) {
    vec3 ftol = fs_in.frag_pos - lpos;

    int samples = 20;
    float diskRadius, bias = 0.15, depth;
    float curDepth = length(ftol), shadow = 0.0;

    diskRadius = (1.0 + (length(cam_pos - fs_in.frag_pos) / farPlane)) / 50.0;

    for (int i = 0; i < samples; ++i) {
        depth = texture(shadow_tex, ftol + kPCFSamples[i] * diskRadius).r;
        shadow += test_gt(curDepth - bias, depth * farPlane);
    }
...
void main(void) {
    ...
    // Apply shadow
    out_col = mix(out_col + ambience, ambience, calc_shadow(light[0].position));
